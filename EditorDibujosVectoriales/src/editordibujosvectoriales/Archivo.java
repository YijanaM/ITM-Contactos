...
public class Archivo {
    ...
    public static void guardarDibujo(String nombreArchivo, ListaLigada listaLigada) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(nombreArchivo))) {
            for (Nodo nodo = listaLigada.cabeza; nodo != null; nodo = nodo.siguiente) {
                Trazo trazo = nodo.trazo;
                String linea = trazo.tipo.toString() + ";" + trazo.x1 + ";" + trazo.y1 + ";" + trazo.x2 + ";" + trazo.y2 + ";255;255;255";
                writer.write(linea);
                writer.newLine();
            }
        } catch (IOException e) {
